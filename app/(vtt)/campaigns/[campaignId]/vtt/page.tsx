...
export default function VttPage() {
  ...
  // Import audio state to show indicator on button
  const { audioState } = useAudioSync(campaignId);
  
  // Shared audio element ref - persists across MusicPlayer mounts/unmounts
  const sharedAudioRef = useRef<HTMLAudioElement | null>(null);
  
  // Keep a stable reference to MusicPlayer to prevent remounting
      ...
      <div className="absolute top-4 right-4 bottom-4 z-10 flex flex-col items-end pointer-events-none">
        ...
        <div className="flex flex-col gap-2 pointer-events-auto bg-background/80 backdrop-blur p-2 rounded-lg border border-white/10 shadow-lg mb-2">
          ...
          >
            ...
            {/* Play/Pause indicator - small dot in corner */}
            {audioState.activeTrackId && (
              <span 
                className={cn(
                ...
                title={audioState.isPlaying ? "Playing" : "Paused"}
              />
            )}
          </Button>
          <Button 
        ...
        {/* Always-mounted audio element - persists across component mounts/unmounts */}
        {/* This ensures audio continues playing even when MusicPlayer UI is not visible */}
